...
from . import mkqa_eval_util as eval_util
...
def read_annotations(gold_path: str) -> Dict[str, Any]:
    ...
    for lang, annotations in all_gold_annotations.items():
        if len(annotations) != 10000:
            # print(f"The annotations file you've provided contains {len(all_gold_annotations)} for language {lang} examples, where 10000 are expected.")
            logging.warning(
...
def mkqa_evaluate(
    ...
    annotations = read_annotations(annotation_file)[language]
    return evaluate(annotations, predictions, language)
