...
def _extract_last_num(text: str) -> float:
...
def msgm_eval(predictions: List[str], gold_labels: List[str]):
    # calculate acc
    correct_results = []
    wrong_results = []
    for prediction, gold_label in zip(predictions, gold_labels):
        result = dict(
            extract_true_num=_extract_last_num(gold_label),
            extract_pred_num=_extract_last_num(prediction),
            is_correct=None,
        )
        if abs(result["extract_true_num"] - result["extract_pred_num"]) < 1e-3:
            result["is_correct"] = True
